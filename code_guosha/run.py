...
for strOption, strArgument in getopt.getopt(sys.argv[1:], '', [ strParameter[2:] + '=' for strParameter in sys.argv[1::2] ])[0]:
    ...
    if strOption == '--scale' and strArgument != '': arguments_scale = strArgument
# end
...
if __name__ == '__main__':
    ...
    tenOne = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(resize(PIL.Image.open(arguments_strOne)))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(resize(PIL.Image.open(arguments_strTwo)))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    ...
    (numpy.array(tenOutput.numpy().transpose(1, 2, 0), numpy.float32) * float(arguments_scale)).tofile(objOutput)
