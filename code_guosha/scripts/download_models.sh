...
for((i=1;i<=1;i++));  
do   
    wget -c -t 0 $BLOB/models/${DATASET}-table1.zip -O $REPO_DIR/models/table1/${DATASET}-table1.zip
...
for((i=1;i<=1;i++));  
do
    wget -c  -t 0 $BLOB/models/${DATASET}-32frm.zip -O $REPO_DIR/models/32frm/${DATASET}-32frm.zip
