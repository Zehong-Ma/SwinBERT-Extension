...
from .data_utils import video_functional as F
import code, time
...
class VisionLanguageTSVDataset(object):
    ...
    def apply_augmentations(self, frames, opticalflow):
        #print(opticalflow.shape)
        #print('---------------------------')
        #print(opticalflow[0].shape)
        ...
        mysize = Resize(self.img_res)
        frame_list =  mysize(frame_list)
        if self.is_train == True:
            mycrop = RandomCrop((self.img_res,self.img_res)) #F.crop_clip(clip, y1, x1, h, w)
            frame_list, y1, x1, h, w = mycrop(frame_list)
            for itr in range(8):
                opticalflow[itr] = opticalflow[itr][y1: y1 + w, x1: x1 + h, : ]
        else:
            mycrop = CenterCrop((self.img_res, self.img_res))
            frame_list = mycrop(frame_list)
            startx = (opticalflow[0].shape[0] - self.img_res) // 2
            starty = (opticalflow[0].shape[1] - self.img_res) // 2
            for itr in range(8):
                opticalflow[itr] = opticalflow[itr][startx: startx + self.img_res, starty: starty + self.img_res,: ]
        ''' 
        mytotensor = ClipToTensor(channel_nb=3)
        mynormalize = Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        framelist = mytotensor(frame_list)
        crop_frames = mynormalize(frame_list)
        '''
        crop_frames = self.raw_video_prcoess(frame_list)
        ...
        self.optical_flow_list = [
            # Resize(self.img_res),
            # CenterCrop((self.img_res,self.img_res)),
            ClipToTensor(channel_nb=2, div_255 = False),
            #Normalize(mean=[0.485, 0.456], std=[0.229, 0.224])
        ]
        self.optical_flow_process = Compose(self.optical_flow_list)
        opticalflow = self.optical_flow_process(opticalflow)

        #print(opticalflow.shape)
        #print(crop_frames.shape)
        #print('???????????????????????????')
        crop_frames = torch.cat((crop_frames, opticalflow), dim=0)
    ...
    def __getitem__(self, idx):
        ...
        # optical flow frames
        videoname = img_key.split('/')[-1].split('.avi')[0]
        output_img_dir = 'datasets/MSVD/opticalflow_8frames/'


        opticalflow = []
        for itr in range(1, 9):
            outputpng = output_img_dir + videoname + '_frame' + str(itr).zfill(4) + '.flo'
            #outputpng = output_img_dir + '0hyZ__3YhZc_289_295'+ '_frame' + str(itr).zfill(4) + '.flo'
            if os.path.exists(outputpng):
                f = open(outputpng, 'rb')
                a=np.fromfile(f, np.uint8, 4)
                f.seek(4, os.SEEK_SET)
                b = np.fromfile(f, dtype=np.int32, count=2)
                f.seek(8, os.SEEK_SET)
                c = np.fromfile(f, dtype=np.float32, count=b[0] * b[1] * 2)
                c = np.reshape(c, (b[1], b[0], 2))
            else:
                if raw_frames.shape[2] < raw_frames.shape[3]:
                    c = np.zeros((self.img_res, int(raw_frames.shape[3] / raw_frames.shape[2] * self.img_res), 2))
                else:
                    c = np.zeros((int(raw_frames.shape[2] / raw_frames.shape[3] * self.img_res), self.img_res, 2))


            opticalflow.append(c)
        ...
        preproc_frames = self.apply_augmentations(raw_frames, opticalflow)
