...
os.environ['CUDA_LAUNCH_BLOCKING']='1' # only debug
import sys
...
from src.datasets.vl_dataloader_clip import make_data_loader
...
from src.modeling.video_captioning_e2e_vid_clip_bert import VideoTransformer
from src.modeling.load_bert_clip import get_bert_model
from src.solver import AdamW, WarmupLinearLR
from src.modeling.clip_model import build_model
...
def mixed_precision_init(args, model):
    ...
    decay_swin_param_tp = [(n, p) for n, p in decay_param_tp if ("clip_model." in n) and (p.requires_grad==True) ]
    decay_bert_param_tp = [(n, p) for n, p in decay_param_tp if "clip_model." not in n]

    no_decay_swin_param_tp = [(n, p) for n, p in no_decay_param_tp if ("clip_model." in n) and (p.requires_grad==True)]
    no_decay_bert_param_tp = [(n, p) for n, p in no_decay_param_tp if "clip_model." not in n]
    # sample_net_params = [(n, p) for n, p in param_optimizer if "sample_net." in n]

    weight_decay = 0.2
    coef_lr = args.backbone_coef_lr 
    # if args.freeze_swinBERT:
    #     
    #     print([n for n, p in sample_net_params])
        
    #     optimizer_grouped_parameters = [
    #         {'params': [p for n, p in sample_net_params], 
    #             'weight_decay': 0,
    ...
    if args.scheduler == "warmup_linear":
        scheduler = WarmupLinearLR(
            optimizer, max_global_step, warmup_ratio=args.warmup_ratio, min_lr=1e-8)
    else:
...
def evaluate(args, val_dataloader, model,tokenizer, output_dir):
    # print("eval_size", args.per_gpu_eval_batch_size)
...
def test(args, test_dataloader, model, tokenizer, predict_file):
    # need to modify because the clip_tokenizer is not used
...
def check_arguments(args):
    ...
    args.max_img_seq_length = args.max_num_frames
...
def update_existing_config_for_inference(args):
    ...
    train_args.max_candidate_frames = args.max_candidate_frames
    train_args.max_num_frames = args.max_num_frames
    return train_args

def get_custom_args(base_config):
    parser = base_config.parser
    parser.add_argument('--max_num_frames', type=int, default=8)
    parser.add_argument('--max_candidate_frames', type=int, default=32)
    ...
    parser.add_argument('--freeze_swinBERT', type=str_to_bool, nargs='?', const=True, default=False)
...
def main(args):
    ...
    clip_model = build_model("ViT-B/16")
    bert_model.set_clip_model(clip_model)
    vl_transformer = VideoTransformer(args, config, bert_model)
    
    vl_transformer.freeze_backbone(freeze=args.freeze_backbone)
    # vl_transformer.freeze_swinBERT(freeze=args.freeze_swinBERT)
    ...
    # freeze SwinBERT
    # for name, param in vl_transformer.named_parameters():
    #     if name.startswith("module"):
    #         name = name[7:]
    #     print(name, param.shape, param.dtype)
    #     if name.startswith("sample_net") is False:
