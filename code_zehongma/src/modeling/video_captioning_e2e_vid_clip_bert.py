...
import random
from src.modeling.sample_net_trainable import SampleNet
...
class VideoTransformer(torch.nn.Module):
    def __init__(self, args, config, transformer_encoder):
        ...
        # self.clip_model = build_model("RN101")
        ...
        if self.learn_mask_enabled==True:
            self.learn_vid_att = torch.nn.Embedding(args.max_num_frames*args.max_num_frames,1)
    ...
    def forward(self, *args, **kwargs):
        ...
        # image_embeddings = self.clip_model.encode_image(images.reshape(-1,C,H,W)).reshape(B,S,-1)
        ...
        # kwargs['img_feats'] = image_embeddings
        # kwargs["text_feats"] = text_embeddings
    ...
    def freeze_backbone(self, freeze=True):
        for n, p in self.trans_encoder.named_parameters():
            # if "clip_model" in n: 
            #     # print(n, not freeze)
            #     p.requires_grad =  not freeze
            
            if "clip_model.transformer" in n or "clip_model.token_embedding" in n or "clip_model.positional_embedding" in n:
                p.requires_grad =  not freeze

    def freeze_fullModel(self, freeze=True):
        for _, p in self.trans_encoder.named_parameters():
            p.requires_grad =  not freeze
        for _, p in self.fc.named_parameters():
            p.requires_grad =  not freeze
        # if self.learn_mask_enabled==True:
        #     self.learn_vid_att.weight.requires_grad=False
