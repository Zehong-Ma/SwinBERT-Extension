...
class VideoTransformer(torch.nn.Module):
    def __init__(self, args, config, swin, transformer_encoder):
        ...
        self.use_sample_net = args.use_sample_net
        if self.use_sample_net:
            
            self.sample_net = SampleNet(args.max_candidate_frames, args.max_num_frames)
    ...
    def forward(self, *args, **kwargs):
        if self.use_sample_net:
            images = self.sample_net(kwargs["img_feats"])
            # images = kwargs['img_feats'][:,::4]
        else:
    ...
    def freeze_swinBERT(self, freeze=True):
        for _, p in self.swin.named_parameters():
            p.requires_grad =  not freeze
