...
class CaptionTensorizer(object):
            ...
            if self.text_mask_type == "attn_on_the_fly" and random.random() > self.random_mask_prob and len(tokens)> 2:
                ...
                for pos in masked_idx:
                    if random.random() <= 0.8:
                        ...
                        if hasattr(self.tokenizer, "get_random_token"):
                            tokens[pos] = self.tokenizer.get_random_token()
                        else:
                            random_index = random.randint(0,49405)
                            tokens[pos] = self.tokenizer.decode(random_index)
                    else:
    ...
    def tokenize_text_inputs(
            ...
            pad_token_segment_id=0, sequence_a_segment_id=272,
            sequence_b_segment_id=273, text_meta=None):
        text_a = self.prepro_raw_txt(text_a)
        if self.is_train:
            tokens_a = self.tokenizer.tokenize(text_a)
            tokens_a = [token.rstrip() for token in tokens_a]
            if self.text_mask_type == "pos_tag":
        ...
        if text_b:
            ...
            tokens_b = [token.rstrip() for token in tokens_b]
    ...
    def tensorize_example_e2e(self, text_a, img_feat, text_b=None,
            cls_token_segment_id=49406, pad_token_segment_id=0,
            sequence_a_segment_id=272, sequence_b_segment_id=273, text_meta=None):
        # tokenize the texts
        ...
        for token in tokens:
            if token[-1]==" " or token[-1]=="\t":
                ...
                precessed_token.append(token[:-1])
            else:
                precessed_token.append(token)
   
        input_ids = self.tokenizer.convert_tokens_to_ids(precessed_token)
        processed_ids = []
        for id in input_ids:
            if id is None:
                processed_ids.append(5542)
            else:
