...
from .modeling_utils_clip import (WEIGHTS_NAME, CONFIG_NAME, PretrainedConfig, PreTrainedModel,
...
class BertConfig(PretrainedConfig):
    r"""
        ...
        Arguments:
            ./xiyin1wu2_maskrcnn_or_config_json_file: Vocabulary size of `inputs_ids` in `BertModel`.
            ...
            type_./xiyin1wu2_maskrcnn: The vocabulary size of the `token_type_ids` passed into
                `BertModel`.
    ...
    def __init__(self,
                 ./xiyin1wu2_maskrcnn_or_config_json_file=49408,
                 hidden_size=768,
                 ...
                 type_./xiyin1wu2_maskrcnn=2,
                 initializer_range=0.02,
                 layer_norm_eps=1e-12,
                 **kwargs):
        super(BertConfig, self).__init__(**kwargs)
        if isinstance(./xiyin1wu2_maskrcnn_or_config_json_file, str) or (sys.version_info[0] == 2
                        and isinstance(./xiyin1wu2_maskrcnn_or_config_json_file, unicode)):
            with open(./xiyin1wu2_maskrcnn_or_config_json_file, "r", encoding='utf-8') as reader:
                json_config = json.loads(reader.read())
            for key, value in json_config.items():
                self.__dict__[key] = value
        elif isinstance(./xiyin1wu2_maskrcnn_or_config_json_file, int):
            self../xiyin1wu2_maskrcnn = ./xiyin1wu2_maskrcnn_or_config_json_file
            ...
            self.type_./xiyin1wu2_maskrcnn = type_./xiyin1wu2_maskrcnn
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = nn.Embedding(config../xiyin1wu2_maskrcnn, config.hidden_size, padding_idx=0)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.token_type_embeddings = nn.Embedding(config.type_./xiyin1wu2_maskrcnn, config.hidden_size)
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = nn.Linear(config.hidden_size,
                                 config../xiyin1wu2_maskrcnn,
                                 bias=False)

        self.bias = nn.Parameter(torch.zeros(config../xiyin1wu2_maskrcnn))
...
class BertImgModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.text_embedding = nn.Linear(self.img_dim, self.config.hidden_size, bias=True)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
            position_ids=None, head_mask=None, img_feats=None, text_feats=None,
            encoder_history_states=None):
        ...
        # pdb.set_trace()
        embedding_output = self.text_embedding(text_feats)
        # add img_embedding_output and sum with embedding_output
        # logger.info('embedding_output: %s' % str(embedding_output.shape))
...
class BertForPreTraining(BertPreTrainedModel):
    r"""
        **masked_lm_labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size, sequence_length)``:
            ...
            Indices should be in ``[-1, 0, ..., config../xiyin1wu2_maskrcnn]`` (see ``input_ids`` docstring)
            Tokens with indices set to ``-1`` are ignored (masked), the loss is only computed for the tokens with labels
            in ``[0, ..., config../xiyin1wu2_maskrcnn]``
    ...
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        ...
        **prediction_scores**: ``torch.FloatTensor`` of shape ``(batch_size, sequence_length, config../xiyin1wu2_maskrcnn)``
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config../xiyin1wu2_maskrcnn), masked_lm_labels.view(-1))
...
class BertForImageCaptioning(BertPreTrainedModel):
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask,
            text_feats=None, masked_pos=None, masked_ids=None,
            masked_pos_img=None, masked_token_img=None,
            token_type_ids=None, position_ids=None, head_mask=None,
            is_training=True, encoder_history_states=None):
        B, S, C, H, W = img_feats.shape  # batch, segment, chanel, hight, width
        img_feats = self.clip_model.encode_image(img_feats.reshape(B*S, C, H, W)).reshape(B,S,-1)
        text_feats = self.clip_model.encode_text(input_ids)

        outputs = self.bert(input_ids, img_feats=img_feats, attention_mask=attention_mask, text_feats=text_feats,
    ...
    def generate(self, img_feats, attention_mask, masked_pos, token_type_ids=None, text_feats=None, 
        ...
        ./xiyin1wu2_maskrcnn = self.config../xiyin1wu2_maskrcnn
        if not use_cbs:
            num_fsm_states = 1
        else:
            b, num_fsm_states, f1, v = fsm.shape
            assert b==batch_size and v==./xiyin1wu2_maskrcnn and f1==num_fsm_states
    ...
    def prod_generate(self, img_feats, od_label_ids, max_length,
        ...
        while True:
            ...
            # Compute scores
            _scores = F.log_softmax(next_token_logits, dim=-1)  # (batch_size, ./xiyin1wu2_maskrcnn)
